...
## LSI模型 **************************************************
# 转化为lsi模型, 可用作聚类或分类
lsi_model = models.lsimodel.LsiModel(corpus_tfidf, id2word=dictionary, num_topics=2)
...
top_one = sorted(enumerate(test_simi), key=lambda item: -item[1])[0]

print('lsi与'+documents[(top_one[0])]+'最像，相似度为：'+str((top_one[1])))
...
print(list(enumerate(test_simi)))
top_one = sorted(enumerate(test_simi), key=lambda item: -item[1])[0]
print('lda与'+documents[(top_one[0])]+'最像，相似度为：'+str((top_one[1])))
